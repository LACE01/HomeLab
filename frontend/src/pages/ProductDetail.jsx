...
export default function ProductDetail() {
  ...
  useEffect(() => { api.get(`/v1/products/${id}`).then(r => setP(r.data)); }, [id]);
  ...
  return (
    <Layout title={p.name} subtitle={p.description}
      actions={<Link to="/products" className="h-8 px-3 text-[12px] border border-[#30363D] rounded inline-flex items-center gap-1.5 text-slate-300"><ArrowLeft size={14}/> Back</Link>}>
